...
namespace Assets.Scripts.Radial_Menu
{
    public class isLvl : MonoBehaviour
    {
        ...
        public void Set2Data(towerData data, float iR, float oR, isLevelTower creator)
        {
            ...
            innerRadius = iR;
            outerRadius = oR;
            normalColor = data.nonHighlightedColor;
            highlightColor = data.highlightedColor;
            disabledColor = data.disabledColor;
            clickEvent = data.onSlicePressed;
            if (parent.fade)
            {
            ...
            sliceIcon.rectTransform.sizeDelta = new Vector2(data.iconSize, data.iconSize);

            sliceLabel.text = data.sliceLabel;
            sliceIcon.sprite = data.icon;
            sliceIcon.transform.position = Center();
            ...
            onHoverEvents = data.hoverFunctions;
            if (onHoverEvents)
            {
                onHoverEnter = data.onHoverEnter;
                onHoverExit = data.onHoverExit;
            }
        }
